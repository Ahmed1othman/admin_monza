...
//            }
    ...
    public function offers() {
        ...
        $cars = Car::with(['images', 'brand', 'model', 'color', 'types', 'company', 'year'])
            ...
            ->whereNotNull('offer_duration')
            ->get()
            ->filter(function ($car) use ($currentTime) {
                // Convert offer_start_time to Carbon instance if it's not null
                if ($car->offer_start_time) {
                    $offerStartTime = \Carbon\Carbon::parse($car->offer_start_time);
                    // Check if the offer is still valid based on offer_duration
                    return $offerStartTime->addHours($car->offer_duration)->greaterThan($currentTime);
                }
                return false;
        });
