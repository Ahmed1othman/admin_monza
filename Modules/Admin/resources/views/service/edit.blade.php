...
@section('content')
            ...
            <div class="layout-px-spacing">
                <form action="{{url('/')}}/admin/service/{{$item->id}}" method="post" enctype="multipart/form-data">
                    @method("PUT")
                    @csrf
                    <div class="page__header_title custom__page_header_title">
                        <h4>{{__('admin.edit')}} {{__('admin.service')}}</h4>
                                        ...
                                        @endforeach
                                            ...
                                            @foreach(\Config::get("app.languages") as $key => $value)
                                                <div class="form-group row mb-4">
                                                    ...
                                                    <div class="col-xl-9 col-lg-9 col-sm-10">
                                                        <textarea  class="form-control" name="description_{{$key}}" >{!!$item->getTranslation('description', $key)!!}</textarea>
                                                    </div>
                                                </div>
                                            @endforeach
